...
/**
 * Project : WebQQCore Package : iqq.im File : WebQQClient.java Author : solosky < solosky772@qq.com > Created :
 * 2012-8-1 License : Apache License 2.0
 */
package iqq.im;
...
public class WebQQClient implements QQClient, QQContext {
    ...
    public WebQQClient(QQNotifyListener notifyListener, QQActorDispatcher actorDispatcher) {
        ...
        this.account.setStatus(QQStatus.ONLINE);
    ...
    public void setHttpProxy(ProxyType proxyType, String proxyHost, int proxyPort, String proxyAuthUser,
        ...
        HttpService http = getSerivce(QQService.Type.HTTP);
        http.setHttpProxy(proxyType, proxyHost, proxyPort, proxyAuthUser, proxyAuthPassword);
    }
    ...
    public QQActionFuture logout(final QQActionListener listener) {
        ...
        return procModule.doLogout(new QQActionListener() {
            public void onActionEvent(QQActionEvent event) {
                ...
                if (event.getType() == QQActionEvent.Type.EVT_OK || event.getType() == QQActionEvent.Type.EVT_ERROR) {
     ...
     */
    @Override
    public QQActionFuture getSelfInfoAction(QQActionListener qqActionListener) {
        UserModule mod = getModule(QQModule.Type.USER);
        return mod.getSelfInfoAction(qqActionListener);
    }
     ...
     */
    @Override
    public QQActionFuture getOffPic(OffPicItem offpic, QQMsg msg, OutputStream picout, QQActionListener listener) {
        ChatModule mod = getModule(QQModule.Type.CHAT);
     ...
     */
    @Override
    public QQActionFuture getUserPic(CFaceItem cface, QQMsg msg, OutputStream picout, QQActionListener listener) {
        ChatModule mod = getModule(QQModule.Type.CHAT);
     ...
     */
    @Override
    public QQActionFuture getGroupPic(CFaceItem cface, QQMsg msg, OutputStream picout, QQActionListener listener) {
        ChatModule mod = getModule(QQModule.Type.CHAT);
     ...
     */
    @Override
    public void relogin(QQStatus status, final QQActionListener listener) {
        ProcModule module = getModule(QQModule.Type.PROC);
        module.relogin(status, listener);
    }
}
